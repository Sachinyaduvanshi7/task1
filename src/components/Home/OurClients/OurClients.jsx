import React from "react";
import BizClass from "./OurClients.module.scss";
import OurPresence from "../OurPresence/OurPresence";
import Footer from "../../Footer/Footer";

const OurClients = () => {
  const OurClients = [
    {
      img: (
        <img
          src="https://mybiznext.in/images/client/client_logo_02.png"
          alt=""
        />
      ),
      P: "Toyota Boshoku Corporation is a Japanese automotive component manufacturer. It is a member of the Toyota Group of companies. The company develops auto components for Toyota Motor's...",
    },
    {
      img: (
        <img
          src="	https://mybiznext.in/images/client/client_logo_03.png"
          alt=""
        />
      ),
      P: "'Kundan Refinery is a most treasured business unit of “Kundan”. Since 18 years, KUNDAN has marked its presence in precious metals..It is well known as largest Gold refinery in private sector in India...",
    },
    {
      img: (
        <img
          src="https://mybiznext.in/images/client/client_logo_04.png"
          alt=""
        />
      ),
      P: "ASG Eye Hospitals, a global leader in ophthalmology, has stood at the forefront of eye care since 2005.ASG Hospital Pvt. Ltd. is one of the largest chain with quality health care services in the field of medicine...",
    },
    {
      img: (
        <img
          src="https://mybiznext.in/images/client/client_logo_05.png"
          alt=""
        />
      ),
      P: "Sanganeriya Spinning Mills Ltd is a leading Manufacturer & Supplier of a wide assortment of Synthetic Yarns. We offer products, which include 100% Acrylic Yarns...",
    },
    {
      img: (
        <img
          src="	https://mybiznext.in/images/client/client_logo_06.png"
          alt=""
        />
      ),
      P: "Musashi Auto Parts (India) Pvt. Ltd is a 100% owned subsidiary of Musashi Seimitsu Industries, Japan. Musashi is a technology driven global leader in precision forging and machining...",
    },
    {
      img: (
        <img
          src="https://mybiznext.in/images/client/client_logo_07.png"
          alt=""
        />
      ),
      P: "Tiger Steel Industires is a wholly owned entity of 'Tiger Metal Industries LLC in UAE ,established in 1972, is one of the first tube manufacturers in the United Arab Emirates ",
    },
    {
      img: (
        <img
          src="data:image/png;base64,"
          alt=""
        />
      ),
    },
    {
      img: (
        <img
          src="data:image/jpeg;base64,"
          alt=""
        />
      ),
    },
    {
      img: (
        <img
          src="https://res.cloudinary.com/crunchbase-production/image/upload/c_lpad,f_auto,q_auto:eco,dpr_1/uerpyg28dnkj59txneel"
          alt=""
        />
      ),
    },
    {
      img: (
        <img
          src="https://images.hindustantimes.com/img/2021/06/30/1600x900/c3ac2bfa-d9d4-11eb-a5ce-a68058c7c394_1625079441804.png"
          alt=""
        />
      ),
    },

    {
      img: (
        <img
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTDkSvroFTtdxHJTNz_nl6znpEHntgLgUI_Xc2Ggr8HUqxmC6S9-X4rvvaOQdJFnxPvLXk&usqp=CAU"
          alt=""
        />
      ),
    },
    {
      img: <img src="http://atekworld.com/imgs/logo.png" alt="" />,
    },
    {
      img: (
        <img
          src="https://upload.wikimedia.org/wikipedia/commons/thumb/4/42/Logo_SMC_Corporation.svg/1200px-Logo_SMC_Corporation.svg.png"
          alt=""
        />
      ),
    },
    {
      img: (
        <img
          src="https://products.rosenberger.com/media/image/Rosenberger_Logo_red_RGB_1000px.png"
          alt=""
        />
      ),
    },
    {
      img: <img src="	https://acacioustech.com/new_aps/img/b5.png" alt="" />,
    },
    {
      img: <img src="https://www.nirmalind.com/img/logo-large.png" alt="" />,
    },

    {
      img: (
        <img
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQjfRvbyeS1RGBm0eN-wQr34cY2BgStiTHvFULDSRd_GtpnU3-7AoO7B7b4oxUxOll4g0c&usqp=CAU"
          alt=""
        />
      ),
    },
  ];
  return (
    <div className={BizClass.OurClients}>
      <div className={BizClass.Top_Section}>
        <div>
          <h4>
            Our Valuable <span>Clients</span>{" "}
          </h4>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. <br />{" "}
            Suscipit iste doloremque fugit, architecto nostrum repudiandae
            voluptas possimus. Quo, iusto itaque.
          </p>
          <button>Learn More</button>
        </div>
        <div>
          <img
            src="https://media.istockphoto.com/id/1370860148/vector/business-handshake-between-man-and-woman.jpg?s=612x612&w=0&k=20&c=js_W3YuyjHuqfDsPqVqwqbUEBKVwSCAZTPOq_oBHXmc="
            alt=""
          />
        </div>
      </div>
      <div className={BizClass.Conatiner}>
        <div>
          {OurClients.map((data, indx) => {
            return (
              <>
                <div className={BizClass.Card} key={indx}>
                  <div className={BizClass.Data_Coantainer}>
                    <p>{data.P}</p>
                  </div>
                  <div className={BizClass.img}>{data.img}</div>
                </div>
              </>
            );
          })}
        </div>
      </div>
      <OurPresence/>
      <Footer/>
    </div>
  );
};

export default OurClients;
